...
Select 
datename(weekday,customer_placed_order_datetime) as week_day,
datepart(hour,customer_placed_order_datetime) as hour,
cast(round(sum(order_total + tip_amount - refunded_amount - discount_amount),2) as decimal(10,2)) as avg_trans_amount
...
Select
concat(datepart(day, customer_placed_order_datetime),'-',datepart(hour, customer_placed_order_datetime)) as day_hour,
sum(order_total + tip_amount - refunded_amount - discount_amount) as trans_amount
from
doordash_delivery
group by concat(datepart(day, customer_placed_order_datetime),'-',datepart(hour, customer_placed_order_datetime))