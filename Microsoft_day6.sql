/*
Microsoft(Medium Level) hashtag#SQL Interview Question — Solution
...
The output should include the project title
and the project budget rounded to the closest integer. Order your list by projects with the highest budget per employee first.
*/

/*
CREATE TABLE ms_projects(id int, title varchar(15), budget int);
INSERT INTO ms_projects VALUES 
(1, 'Project1',  29498),(2, 'Project2',  32487),
...
Select 
p.title, round(p.budget,0) as budget
from
ms_projects as p
left join
ms_emp_projects as e
on p.id = e.project_id
group by
p.title, p.id, p.budget
order by
p.budget desc
