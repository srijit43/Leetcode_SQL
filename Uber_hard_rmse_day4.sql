...
(defined as distance_to_travel/monetary_cost), first sum the "distance to travel" and "monetary cost" values monthly. 
...
/*CREATE TABLE uber_request_logs(request_id int, request_date datetime, 
request_status varchar(10), distance_to_travel float, monetary_cost float, driver_to_client_distance float);
...
Select
convert(char(7), request_date, 120) as year_month,
SUM(distance_to_travel) as total_distance,
SUM(monetary_cost) as total_cost
from uber_request_logs
group by convert(char(7), request_date, 120)),

distance_per_dollar as
(Select year_month, total_distance / total_cost as distance_per_dollar
from monthly_aggregates),

naive_forecast as
(Select year_month, distance_per_dollar, 
LAG(distance_per_dollar,1) over (order by year_month) as forecasted_value
from distance_per_dollar)

Select 
round(sqrt(avg(power((distance_per_dollar - forecasted_value),2))),2) as rmse
from naive_forecast
