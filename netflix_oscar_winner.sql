...
If there are more than one person with the same number of oscar wins, return the first one in alphabetic 
order based on their name. Use the names as keys when joining the tables.*/
...
(
Select o.nominee, count(*) as total_wins
from
oscar_nominees o
where o.winner = 1
group by
